...
- [flink-ml-predict.md](flink-ml-predict.md) — Confluent Flink SQL `ML_PREDICT()` function: model connection setup, CREATE MODEL syntax, LATERAL TABLE join pattern, supported providers (OpenAI, Bedrock, Vertex AI, Anthropic, Fireworks AI, plus Confluent-managed open-weight models), PII masking before API calls, and rate-limit considerations.
